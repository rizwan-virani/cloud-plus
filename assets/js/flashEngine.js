/* ============================================================================
   cloud+    ::  flashEngine.js  —  Rapid Recall Flashcards
   ...
   - Cards live in lazy-loaded per-domain modules (assets/js/content/flashN.js),
     each populating CLOUDPLUS.flash[N] with 100 cards { id, d, t, obj, front, back }.
   - Scheduling state persists to localStorage (cloudplus.flash.v1) keyed by card id,
...
(function () {
  ...
  var DECK_SIZE = 100;       // fallback deck size if domainMeta omits a count
  ...
  /* Domain set + per-domain deck sizes come from domainMeta so the engine scales
     to any number of domains (CV0-004 has six) without hardcoded 5/500 totals. */
  function domainIds() { return (S.domainMeta || []).map(function (d) { return d.id; }); }
  function deckSize(n) { var m = (S.domainMeta || []).filter(function (d) { return d.id === n; })[0]; return (m && m.cards) ? m.cards : DECK_SIZE; }
  function totalCards() { return (S.domainMeta || []).reduce(function (a, d) { return a + (d.cards || DECK_SIZE); }, 0) || 500; }

  function ver() { return window.CLOUDPLUS_VER ? "?v=" + window.CLOUDPLUS_VER : ""; }
  ...
  window.FLASH = {
    ...
    domainDue: function (n) {
      ...
      return deckSize(n) - notDue; // unseen + due
    },
    allDue: function () {
      var st = store(), notDue = 0;
    ...
    analytics: function () {
      ...
      var byD = {}; var self = this; domainIds().forEach(function (n) { byD[n] = { total: deckSize(n), studied: 0, mastered: 0, due: self.domainDue(n) }; });
      for (var id in st) {
