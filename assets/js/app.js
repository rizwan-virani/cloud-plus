/* ============================================================================
   ...
   Architecture
   ------------
   - contentData.js loads first and defines the global `CLOUDPLUS` namespace with
...
(function () {
  ...
  var Store = (function () {
    ...
    return {
      ...
      get: function (k, fb) { return read("cloudplus." + k, fb); },
      set: function (k, v) { write("cloudplus." + k, v); },
      del: function (k) { try { localStorage.removeItem("cloudplus." + k); } catch (e) {} }
    };
  })();
  ...
  function renderDashboard() {
    ...
    b1.appendChild(el("p.sectsub", { text: "Start here. Understand how the CV0-004 is scored and delivered, and where the credential fits in an IT/cloud career." }));
    var g1 = el("div.grid");
    g1.appendChild(card({
      icon: "🎓", tag: "OVERVIEW", title: "Exam Mechanics", cls: "",
      desc: "Format, timing, scoring, and question types for the CV0-004 &mdash; the high-stakes details every candidate should internalize before sitting the test.",
    ...
    g1.appendChild(card({
      ...
      desc: "Where Cloud+ sits on the career ladder: DoD 8140 work-role alignment, and the cloud engineer, sysadmin, DevOps, and cloud security analyst roles it unlocks.",
    ...
    if (window.FLASH) {
      ...
      gf2.appendChild(card({
        ...
        desc: "Every Cloud+ acronym across all six domains in one deck &mdash; the highest-yield rote material on the exam.",
    ...
    b4.appendChild(el("p.sectsub", { html: "Drawn at random from a bank of <b>" + (S.questions ? S.questions.length : "500+") + "</b> original items, graded with full rationale. Domain quizzes isolate one objective set; the quick quiz mixes all six." }));
    ...
    b5.appendChild(el("p.sectsub", { html: "A realistic, full-length simulation of the CV0-004 &mdash; 90 questions weighted to the official domain percentages. Sit it under timed conditions to benchmark your readiness." }));
    var g3c = el("div.grid");
    ...
    g5.appendChild(card({
      ...
      desc: "Vetted, free study tools: official CompTIA CV0-004 objectives, cloud-provider free tiers, NIST cloud definitions, and trusted community guides.",
  ...
  /* ===========================================================================
     ...
     =========================================================================== */
  function certVer() { var v = window.CLOUDPLUS_VER || ""; return v ? "?v=" + v : ""; }
  ...
  function renderCertCatalog(mount) {
    ...
    var search = el("input.cert-search", { type: "text", placeholder: "e.g. Cloud+" });
  ...
  /* Lazily fetch a domain's reading module the first time it's opened, so the
     ...
     script populates CLOUDPLUS.reading[id]; once cached it renders instantly. */
  ...
  function loadDomainModule(id, onReady, onFail) {
    ...
    s.src = "assets/js/content/domain" + id + ".js" + (window.CLOUDPLUS_VER ? "?v=" + window.CLOUDPLUS_VER : "");
    s.async = true;
    s.onload = function () {
  ...
  function openDomain(id) {
    ...
    function render() {
      ...
      Router.open(readerView("Domain " + id, d.title, "CV0-004 Domain " + id + " · " + d.weight + "% of the exam", sections));
    }
  ...
  function openExamMechanics() { Router.open(readerView("Exam mechanics", "Exam Mechanics: CV0-004", "How the exam is built, delivered, and scored", S.examMechanics || [])); }
  function openCareer() { Router.open(readerView("Career guidance", "Career Guidance", "Where Cloud+ fits in your path", S.careerGuidance || [])); }
